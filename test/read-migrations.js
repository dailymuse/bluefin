...
describe("read-migrations", () => {
    it("reads migrations", async function() {
        await Sequence.readMigrations(vfs, "/test/alpha").then(s => s.programs.forEach(p => p.must.be.a(Migration)));
    ...
    it("rejects missing migration after last", function() {
        return Sequence.readMigrations(vfs, "/test/eta", undefined, 1).must.reject.an(Error);
    });
});

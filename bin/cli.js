...
function readOptions(cmd) {
    ...
    if (cmd.parent.migration) {
        options.last = cmd.parent.migration;
    }
    if (cmd.parent.list) {
        options.list = cmd.parent.list;
    }
    return options;
}
...
function rebuild(dbName, schemaName, cmd) {
    ...
    readConf(cmd.parent.conf)
        ...
        .then(db => {
            const vow = schemaName ? db.rebuildSchema(schemaName, options) : db.rebuild(options);
...
function apply(dbName, schemaName, cmd) {
    ...
    readConf(cmd.parent.conf)
        .then(conf => conf.database(dbName))
        .then(db => {
            const vow = schemaName ? db.applySchema(schemaName, options) : db.apply(options);
            return vow.finally(() => db.disconnect());
        })
        .catch(err => {
            // tslint:disable-next-line:no-console

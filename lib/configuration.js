...
export default class Configuration {
  ...
  migrations (relative) {
    const absolute = path.resolve(this.directory, relative)
    return Promise.fromCallback(cb => this.fs.readdir(absolute, cb))
      .then(files => {
        const vows = files.map(ea => {
          const filePath = path.resolve(absolute, ea)
          return Program.fromFile(this.fs, filePath)
        })
        return Promise.all(vows)
      }).then(programs => new Sequence(...programs))
  }

...
export default class Schema extends Entity {
    ...
    apply(client, options = {}) {
        ...
        const vow = options.first
            ...
            : this.getOrdinalOfLastAppliedMigration(client).then(ordinal => ordinal + 1);
    ...
    build(client, options) {
        ...
        return this.create(client).then(() => this.apply(client, buildOpts));
    }
    ...
    drop(client) {
        return client.exec(`DROP SCHEMA IF EXISTS ${this.name} CASCADE`).then(() => {
        ...
        return this.exists(client).then(exists => (!exists ? this.create(client) : undefined));
    }
    ...
    grantPrivileges(client, options) {
        ...
        this.userNames.forEach(u => {
            ...
            paths.forEach(p => {
                vow = vow.then(() => {
                    return this.conf.program(p).then(program => program.exec(client, context, options));
                });
            });
        });
    ...
    rebuild(client, options) {
        return this.drop(client).then(() => this.build(client, options));
    }

...
export default class Schema extends Entity {
  ...
  apply(client, options = {}) {
    ...
    const vow = options.first
      ...
      : this.getOrdinalOfLastAppliedMigration(client).then(
          ordinal => ordinal + 1
        );
  ...
  drop(client) {
    return client
      .exec(`DROP SCHEMA IF EXISTS ${this.name} CASCADE`)
      .then(() => {
    ...
    return this.exists(client).then(exists =>
      !exists ? this.create(client) : undefined
    );
  }

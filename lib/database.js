...
export default class Database extends Entity {
    ...
    connect() {
        if (!this._dbClientVow) {
            this._dbClientVow = this.cluster.connect(this.name);
        }
    ...
    apply(options) {
        return this.connect().then(client => {
            ...
            for (let name in this.schema) {
                ...
                vow = vow.then(() => schema.apply(client, options)).then(() => schema.grantPrivileges(client, options));
            }
    ...
    drop() {
        if (this.raw.safety) {
            return Promise.reject(new Error("safety"));
        }

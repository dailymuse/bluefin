  ...
  rebuild () {
    ...
    return this.ensure()
      ...
      .then(c => {
        ...
        // drop the schemata defined by the conf
        for (let name in this.schema) {
          const s = this.schema[name]
          vow = vow.then(() => s.drop(c))
        }

        // rebuild the bluefin schema
        const bluefin = Schema.bluefin()
        vow = vow.then(() => bluefin.drop(c))
        vow = vow.then(() => bluefin.build(c))

        // build the schemata defined by the conf
        for (let name in this.schema) {
          const s = this.schema[name]
          vow = vow.then(() => s.build(c))
        }
